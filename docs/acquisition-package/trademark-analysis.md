# Solvy — Registrazione del Marchio: Analisi Costo/Beneficio

*Non una raccomandazione automatica a registrare. Un'analisi che confronta costo, tempi e beneficio reale in una cessione di asset tecnologico — con una scoperta concreta che cambia la valutazione rispetto a un'analisi puramente teorica.*
...
| Opzione | Costo indicativo | Copertura |
|---|---|---|
| Marchio nazionale italiano (UIBM) | ~€100-200 di tasse ufficiali per una classe | Solo Italia — lascia la Spagna scoperta, incoerente con un prodotto IT+ES |
| Marchio dell'Unione Europea (EUTM, EUIPO) | ~€850-1000 di tasse ufficiali per una classe (fino a 3 incluse nella tassa base), più eventuali onorari di un consulente in proprietà industriale (variabili, spesso alcune centinaia di euro aggiuntivi) | Tutti i 27 stati UE, incluse Italia e Spagna — l'opzione coerente con il mercato reale di Solvy |

Cifre indicative basate sulla struttura tariffaria pubblica EUIPO, da confermare con un consulente al momento della decisione — non sono state verificate da questa sede contro il tariffario ufficiale aggiornato.

## Tempi

Una domanda EUTM non opposta richiede tipicamente **4-6 mesi** dal deposito alla registrazione (esame formale + periodo di opposizione di terzi di 3 mesi). Se qualcuno si oppone — scenario reso più plausibile dalla scoperta sopra — i tempi si estendono facilmente a **8-12+ mesi**, con esito non garantito.

Questo tempo **quasi certamente non si allinea con i tempi realistici di un closing**: una registrazione avviata oggi non sarà quasi certamente completa quando la trattativa si chiude.
...
- Il marchio diventa un asset concreto solo **se e quando** un acquirente dichiara esplicitamente l'intenzione di continuare a operare sotto lo stesso nome — a quel punto, e solo a quel punto, la registrazione elimina un rischio reale (che un terzo registri il nome dopo il closing e costringa un rebrand forzato, anche dopo mesi di investimento nel nome)
