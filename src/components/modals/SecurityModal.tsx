...
const SecurityModal = ({ isOpen, onClose, darkMode }: SecurityModalProps) => {
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center p-4 sm:items-center">
          ...
          <motion.div initial={{ opacity: 0, y: 100, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 100, scale: 0.95 }} className={`relative w-full max-w-sm rounded-[32px] p-6 shadow-2xl space-y-6 overflow-hidden transition-colors ${darkMode ? 'bg-slate-900' : 'bg-white'}`}>
            ...
            <div className="space-y-3">
              <div className={`p-4 rounded-2xl flex items-center justify-between transition-colors ${darkMode ? 'bg-slate-800' : 'bg-slate-50'}`}>
                ...
                <button onClick={() => setFaceIdEnabled(!faceIdEnabled)} className={`w-12 h-6 rounded-full transition-all relative active:scale-95 ${faceIdEnabled ? 'bg-primary shadow-lg shadow-primary/30' : (darkMode ? 'bg-slate-700' : 'bg-slate-200')}`}>
