        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: 'spring', stiffness: 300, damping: 25 }} className="relative w-full max-w-sm rounded-[32px] p-6 shadow-2xl space-y-6 overflow-hidden transition-colors" style={{ backgroundColor: 'var(--color-card)' }}>
            <div className="flex items-center justify-between">
              <h3 className={`text-xl font-bold transition-colors ${darkMode ? 'text-white' : 'text-slate-900'}`}>{t('security_modal.title')}</h3>
            ...
            <div className="space-y-3">
              <div className={`p-4 rounded-2xl flex items-center justify-between transition-colors ${darkMode ? 'bg-slate-800' : 'bg-slate-50'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500">{t('security_modal.faceid_subtitle')}</p>
                  </div>
                </div>
              ...
              <button onClick={handlePasswordReset} className={`w-full p-4 border rounded-2xl flex items-center justify-between group active:scale-[0.98] transition-all ${darkMode ? 'bg-slate-900 border-slate-800 hover:border-primary/40 hover:shadow-lg hover:shadow-primary/10' : 'bg-white border-slate-100 hover:border-primary/20 hover:shadow-lg hover:shadow-primary/5'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-[10px] text-slate-500">{t('security_modal.password_subtitle')}</p>
                  </div>
                </div>
              ...
              <button onClick={handleTerms} className={`w-full p-4 border rounded-2xl flex items-center justify-between group active:scale-[0.98] transition-all hover:shadow-lg ${darkMode ? 'bg-slate-900 border-slate-800 hover:border-primary/40 hover:shadow-primary/10' : 'bg-white border-slate-100 hover:border-primary/20 hover:shadow-primary/5'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className={`text-sm font-bold transition-colors ${darkMode ? 'text-white' : 'text-slate-900'}`}>{t('settings.terms_of_service')}</p>
                    <p className="text-[10px] text-slate-500">{t('security_modal.terms_subtitle')}</p>
                  </div>
                </div>
                <ChevronRight size={18} className="text-slate-300 group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
            <button onClick={onClose} className={`w-full py-4 rounded-2xl font-bold shadow-xl active:scale-95 transition-all ${darkMode ? 'bg-white text-slate-900 shadow-slate-900/20' : 'bg-slate-900 text-white shadow-slate-200'}`}>{t('security_modal.close_btn')}</button>
          </motion.div>
        </div>
      )}
    </AnimatePresence>
