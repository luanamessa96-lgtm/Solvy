        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
            ...
            <div className={`flex items-start justify-between p-6 pb-4 shrink-0 ${darkMode ? 'border-b border-slate-800' : 'border-b border-slate-100'}`}>
              <div>
                <h2 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-slate-900'}`}>{isProforma ? t('documents.new_proforma') : t('documents.new_invoice')}</h2>
                <p className="text-sm text-slate-500">{form.invoiceNumber || nextInvoiceNumber}</p>
              </div>
              <button onClick={onClose} aria-label="Chiudi" className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${darkMode ? 'bg-slate-800 text-slate-400 hover:text-white' : 'bg-slate-100 text-slate-500 hover:text-slate-700'}`}>
