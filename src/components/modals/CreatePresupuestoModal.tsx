...
const CreatePresupuestoModal = ({ isOpen, onClose, onSave, profile, documents, darkMode }: CreatePresupuestoModalProps) => {
  ...
  const nextNumber = useMemo(() => {
    const count = documents.filter(d => d.type === 'presupuesto' && new Date(d.date).getFullYear() === currentYear).length + 1;
    return `PRS${String(count).padStart(3, '0')}/${currentYear}`;
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl backdrop-blur-xl max-h-[90dvh] grid grid-rows-[auto_1fr]"
          ...
          >
            <div className={`flex items-start justify-between p-6 pb-4 shrink-0 ${darkMode ? 'border-b border-slate-800' : 'border-b border-slate-100'}`}>
              <div>
                <h2 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-slate-900'}`}>Nuevo Presupuesto</h2>
                <p className="text-sm text-slate-500">{nextNumber}</p>
              </div>
            ...
            <div className="overflow-y-auto overscroll-y-contain p-6 space-y-5 [padding-bottom:max(3rem,calc(env(safe-area-inset-bottom)+2rem))]">
              ...
              {base > 0 && (
                <div className={`rounded-2xl p-4 space-y-2 ${darkMode ? 'bg-slate-800' : 'bg-slate-50'}`}>
                  ...
                  <div className={`flex justify-between pt-2 border-t ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                    <span className={`text-sm font-bold ${darkMode ? 'text-white' : 'text-slate-900'}`}>Total estimado</span>
                    <span className="text-sm font-bold text-amber-500">€{total.toFixed(2)}</span>
                  </div>
                </div>
              )}
              ...
              <div className={`flex items-start gap-3 p-4 rounded-2xl border ${darkMode ? 'bg-amber-500/10 border-amber-500/20' : 'bg-amber-50 border-amber-100'}`}>
                ...
                <p className={`text-xs leading-relaxed ${darkMode ? 'text-amber-300' : 'text-amber-800'}`}>
                  <span className="font-bold">Documento no fiscal</span> — El presupuesto no cuenta para el facturado y no tiene validez fiscal. Los cálculos mostrados son estimaciones. Consulta siempre con tu gestor o asesor fiscal.
                </p>
              </div>
