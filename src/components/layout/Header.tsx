...
const Header = ({ title, activeProfile, onProfileClick, onBellClick, notificationCount, showBack, onBack, darkMode }: HeaderProps) => (
  <header className={`sticky top-0 z-20 backdrop-blur-md px-6 py-4 flex items-center justify-between border-b transition-colors duration-500 ${darkMode ? 'bg-slate-950/80 border-slate-800' : 'bg-white/80 border-slate-100'}`}>
    <div className="flex items-center gap-3">
      {showBack ? (
        <button onClick={onBack} className={`p-2 -ml-2 rounded-xl active:scale-90 transition-all hover:shadow-lg ${darkMode ? 'bg-slate-900 text-slate-300 hover:shadow-primary/10' : 'bg-slate-50 text-slate-600 hover:shadow-slate-200'}`}>
          <ArrowLeft size={20} />
        </button>
      ) : (
    ...
    <div className="flex items-center gap-4">
      ...
      <button onClick={onProfileClick} className={`w-9 h-9 rounded-full border-2 overflow-hidden transition-all active:scale-90 hover:shadow-lg ${darkMode ? 'border-slate-800 hover:border-primary hover:shadow-primary/20' : 'border-slate-100 hover:border-primary hover:shadow-slate-200'}`}>
