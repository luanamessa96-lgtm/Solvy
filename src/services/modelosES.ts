...
export async function generateGastosTrimestreBlob(
  ...
  // ── JUSTIFICANTES FOTOGRÁFICOS ────────────────────────────────────────────
  // Per ogni gasto del trimestre che ha un'immagine allegata, aggiunge una pagina
  const docsWithImages = docs.filter(d => d.imageData);
  for (let i = 0; i < docsWithImages.length; i++) {
    const doc = docsWithImages[i];
    const refNum = `FREC${String(docs.indexOf(doc) + 1).padStart(3, '0')}/${year}`;

    pdf.addPage();

    // Mini header
    pdf.setFont('helvetica', 'bold'); pdf.setFontSize(11); pdf.setTextColor(...black);
    pdf.text('JUSTIFICANTE', M, 12);
    pdf.setFont('helvetica', 'normal'); pdf.setFontSize(8.5); pdf.setTextColor(...grey);
    const fechaStr = new Date(doc.date).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' });
    pdf.text(`${refNum} · ${fechaStr} · ${doc.title || doc.category || '—'}`, M, 19);
    const totalDoc = doc.amount + doc.amount * ((doc.ivaRate ?? 0) / 100);
    pdf.setFont('helvetica', 'bold'); pdf.setFontSize(9); pdf.setTextColor(...primary);
    pdf.text(fmtES(totalDoc), R, 19, { align: 'right' });
    pdf.setDrawColor(...lightGrey); pdf.setLineWidth(0.4); pdf.line(M, 22, R, 22);

    // Recupera immagine (base64 diretta o URL remota da Supabase)
    let imgData = doc.imageData!;
    ...
    const fmt = mimeMatch?.[1]?.toUpperCase() === 'PNG' ? 'PNG' : 'JPEG';

    // Scala proporzionalmente per riempire la pagina
    await new Promise<void>(resolve => {
      const img = new Image();
      img.onload = () => {
        const maxW = R - M;            // ~273mm
        const maxH = 210 - 26 - 16;   // ~168mm (spazio header + footer)
        const ratio = img.naturalWidth / img.naturalHeight;
        let w = maxW;
        let h = w / ratio;
        if (h > maxH) { h = maxH; w = h * ratio; }
        const x = M + (maxW - w) / 2;
        try { pdf.addImage(imgData, fmt, x, 25, w, h); } catch { /* immagine malformata */ }
        resolve();
      };
      img.onerror = () => resolve();
      img.src = imgData;
    ...
    // Footer
    pdf.setDrawColor(...lightGrey); pdf.setLineWidth(0.3); pdf.line(M, 196, R, 196);
    pdf.setFont('helvetica', 'italic'); pdf.setFontSize(6.5); pdf.setTextColor(...grey);
    pdf.text('Generado con Solvy. Estimación indicativa — verifica con tu gestor o asesor fiscal.', M, 200);
    pdf.text(`Generado el ${new Date().toLocaleDateString('es-ES')} · T${quarter} ${year}`, R, 200, { align: 'right' });
  }
