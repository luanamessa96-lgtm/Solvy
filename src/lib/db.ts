...
export async function getProfiles(userId: string, userEmail?: string): Promise<Profile[]> {
  ...
  return (data || []).map(p => ({
    ...
    coefficiente: p.coefficiente != null ? Number(p.coefficiente) : undefined,
    annoInizioAttivita: p.anno_inizio_attivita != null ? Number(p.anno_inizio_attivita) : undefined,
    meseInizioAttivita: p.mese_inizio_attivita != null ? Number(p.mese_inizio_attivita) : undefined,
    iban: p.iban,
    ...
    territory: (p.territory as 'peninsula' | 'canarias') ?? 'peninsula',
...
export async function createProfile(profile: Profile): Promise<void> {
  ...
  const { error } = await getClient()
    ...
    .insert({
      ...
      mese_inizio_attivita: profile.meseInizioAttivita ?? null,
      iban: profile.iban,
      is_pro: profile.isPro ?? false,
      regimen_fiscal: profile.regimenFiscal ?? null,
      iva_habitual: profile.ivaHabitual ?? null,
      region: profile.region ?? null,
      territory: profile.territory ?? 'peninsula',
      street: profile.street ?? null,
      cap: profile.cap ?? null,
      city: profile.city ?? null,
      province: profile.province ?? null,
      reddito_n1: profile.redditoN1 ?? null,
      invoice_counters: profile.invoiceCounters ?? null,
      deleted_invoice_numbers: profile.deletedInvoiceNumbers ?? null,
      has_ostativa_cause: profile.hasOstativaCause ?? null,
    });
