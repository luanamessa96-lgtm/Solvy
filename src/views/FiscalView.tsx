...
const FiscalView = ({ profile, onUpdateProfile, darkMode, documents = [] }: FiscalViewProps) => {
  ...
  const retaAlert = useMemo(() => {
    ...
    if (profile.redditoN1 != null && profile.redditoN1 > 0) {
      monthlyNet = profile.redditoN1 / 12;
    } else {
      const yearDocs = documents.filter(d => new Date(d.date.split('T')[0]).getFullYear() === currentYear);
      ...
      monthlyNet = (annualIncome - annualExpenses) / 12;
    }
            ...
            <div className="p-4 rounded-3xl border space-y-3 transition-colors" style={{ backgroundColor: 'var(--color-card)', borderColor: 'var(--color-border)' }}>
              ...
              <div className="flex gap-2 items-stretch">
                ...
                <button
                  ...
                  className={`self-stretch px-4 rounded-xl text-xs font-bold transition-all active:scale-95 ${redditoN1Saved ? 'bg-emerald-500 text-white' : 'bg-primary text-white'}`}
                >
          ...
          <motion.div variants={item}>
            <div className={`flex items-start gap-3 px-4 py-3.5 rounded-2xl border ${darkMode ? 'bg-slate-800/60 border-slate-700 text-slate-400' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
              ...
              <p className="text-[11px] leading-relaxed">
                Las previsiones cubren estimación directa estándar. ¿Situación especial (deporte, cooperativa, jubilación)? Algunos cálculos podrían variar — estamos trabajando en ello. Háblalo con tu asesor fiscal.
              </p>
            </div>
          </motion.div>
