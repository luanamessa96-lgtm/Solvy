...
const MenuView = ({ activeProfile, onProfileClick, onSettingsClick, onSubscriptionClick, onAccountantClick, onFiscalClick, onGuidaFiscaleClick, onGuiaFiscalESClick, onLogout, darkMode }: MenuViewProps) => {
  ...
  const menuItems: { label: string; icon: LucideIcon; onClick: () => void; color?: string }[] = [
    { label: activeProfile.country === 'Spain' ? 'Fiscalidad' : 'Fiscalità', icon: BarChart2, onClick: onFiscalClick },
    ...
    { label: t('menu.subscription'), icon: CreditCard, onClick: () => isPro ? onSubscriptionClick() : setIsPaywallOpen(true) },
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="p-6 space-y-6 pb-24">
      ...
      {isPro ? (
        <div className={`p-5 rounded-3xl flex items-center gap-4 border transition-colors ${darkMode ? 'bg-primary/10 border-primary/20' : 'bg-primary/5 border-primary/15'}`}>
          ...
          <button onClick={onSubscriptionClick} className={`text-xs font-bold text-primary shrink-0 active:scale-90 transition-all ${darkMode ? 'hover:text-primary/80' : ''}`}>
            {t('menu.manage')}
          </button>
        </div>
      ) : (
