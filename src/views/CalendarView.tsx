...
const CalendarView = ({ deadlines, onAddDeadline, onUpdateDeadline, onDeleteDeadline, darkMode, profile, openAddTrigger, income, expenses, documents }: CalendarViewProps) => {
  ...
  const [newDeadline, setNewDeadline] = useState({ title: '', date: todayLocalISO(), type: 'tax' as Deadline['type'], amount: '' });
        ...
        <motion.div variants={container} className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {months.map((month, index) => {
              const hasFiscal = scadenzeFiscaliRaw.some(s => getLocalMonth(s.date) === index);
              const hasCustomDeadlines = !isSpain && yearDeadlines.some(d => !fiscalTitleSet.has(d.title) && getLocalMonth(d.date) === index);
              ...
              return (
                <motion.button variants={item} key={month} onClick={() => { setSelectedMonth(isSelected ? null : index); setViewMode('list'); }} className={`relative p-4 rounded-3xl border transition-all text-center space-y-1 active:scale-[0.95] ${isSelected ? 'bg-primary border-primary text-white shadow-xl shadow-primary/40' : (darkMode ? 'bg-slate-900 border-slate-800 text-slate-400 hover:border-primary/40 hover:shadow-lg hover:shadow-primary/10' : 'bg-white border-slate-100 text-slate-600 hover:border-primary/20 hover:shadow-lg hover:shadow-primary/5')}`}>
                  ...
                  {!isSpain && hasFiscal && !isSelected && <div className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full bg-primary" />}
                  {hasCustomDeadlines && !isSelected && <div className="absolute top-2 right-5 w-1.5 h-1.5 rounded-full bg-blue-400" />}
                  {isSpain && hasFiscal && !isSelected && <div className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full bg-red-500" />}
            ...
            <div className="space-y-3">
              {filteredDeadlines.length > 0 ? filteredDeadlines.map(deadline => (
                <motion.button variants={item} key={deadline.id} onClick={() => { if (!deadline.id.startsWith('reta-virtual-') && !deadline.id.startsWith('fiscal-virtual-')) setSelectedDeadline(deadline); }} className={`w-full p-4 border rounded-2xl flex items-center gap-4 transition-all active:scale-[0.98] hover:shadow-xl text-left ${deadline.completed ? 'opacity-50' : ''} ${darkMode ? 'bg-slate-900 border-slate-800 hover:border-primary/40 hover:shadow-primary/10' : 'bg-white border-slate-100 hover:border-primary/20 hover:shadow-primary/5'}`}>
                  <div className={`w-12 h-12 rounded-xl flex flex-col items-center justify-center shrink-0 ${deadline.completed ? (darkMode ? 'bg-slate-800 text-slate-500' : 'bg-slate-100 text-slate-400') : Math.ceil((parseLocalDate(deadline.date).getTime() - new Date().getTime()) / 86400000) < 0 ? (darkMode ? 'bg-slate-800 text-slate-500' : 'bg-slate-100 text-slate-400') : fiscalTitleSet.has(deadline.title) ? (darkMode ? 'bg-red-500/10 text-red-500' : 'bg-red-50 text-red-600') : (darkMode ? 'bg-blue-500/10 text-blue-500' : 'bg-blue-50 text-blue-600')}`}>
                          ...
                          {(() => {
                              ...
                              return (
                                <>
                                  ...
                                  <span className={`text-[10px] font-bold uppercase tracking-wider ${daysLeft < 0 ? 'text-slate-400' : 'text-blue-500'}`}>
                                    {daysLeft < 0 ? t('documents.status_badge_overdue') : t('calendar.expiring')}
                                  </span>
                                </>
                              );
                            }
                            ...
                            if (daysToDeadline < 0) {
                              return (
                                <>
                                  <div className={`w-1 h-1 rounded-full ${darkMode ? 'bg-slate-800' : 'bg-slate-200'}`} />
                                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('documents.status_badge_overdue')}</span>
                                </>
                              );
                            }
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: 'spring', stiffness: 300, damping: 25 }} className="relative w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl backdrop-blur-xl" style={{ backgroundColor: 'var(--color-card)' }}>
              <div className="p-8 space-y-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="space-y-1.5">
                      ...
                      <input type="date" value={newDeadline.date} onChange={e => setNewDeadline({ ...newDeadline, date: e.target.value })} className={`w-full appearance-none px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-100 text-slate-900'}`} />
                    </div>
                    <div className="space-y-1.5">
                ...
                <button onClick={() => { if (!newDeadline.title) return; onAddDeadline({ id: Math.random().toString(36).substr(2, 9), title: newDeadline.title, date: newDeadline.date, type: newDeadline.type, amount: newDeadline.amount ? parseFloat(newDeadline.amount) : undefined }); setIsAddOpen(false); setNewDeadline({ title: '', date: todayLocalISO(), type: 'tax', amount: '' }); }} className="w-full bg-primary text-white py-4 rounded-2xl font-bold shadow-xl shadow-primary/30 active:scale-[0.98] transition-all">{t('calendar.add_deadline_confirm')}</button>
              </div>
            </motion.div>
          </div>
        )}
              ...
              <div className="p-8 space-y-5">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">{t('calendar.add_modal_subtitle')}</p>
                  </div>
                ...
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">{t('calendar.field_title')}</label>
                    <input type="text" value={deadlineToEdit.title} onChange={e => setDeadlineToEdit({ ...deadlineToEdit, title: e.target.value })} placeholder={t('calendar.deadline_placeholder')} className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 ${darkMode ? 'bg-slate-800 border-slate-700 text-white placeholder:text-slate-600' : 'bg-slate-50 border-slate-100 text-slate-900 placeholder:text-slate-400'}`} />
                  </div>
                  <div className="flex flex-col gap-3">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">{t('calendar.field_date')}</label>
                      <input type="date" value={deadlineToEdit.date} onChange={e => setDeadlineToEdit({ ...deadlineToEdit, date: e.target.value })} className={`w-full appearance-none px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-100 text-slate-900'}`} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">{t('calendar.field_amount')}</label>
                      <input type="number" value={deadlineToEdit.amount || ''} onChange={e => setDeadlineToEdit({ ...deadlineToEdit, amount: parseFloat(e.target.value) || undefined })} placeholder="0.00" className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 ${darkMode ? 'bg-slate-800 border-slate-700 text-white placeholder:text-slate-600' : 'bg-slate-50 border-slate-100 text-slate-900 placeholder:text-slate-400'}`} />
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">{t('calendar.field_type')}</label>
                    <div className="grid grid-cols-3 gap-2">
                      {(['tax', 'payment', 'other'] as const).map(type => (
                        <button key={type} onClick={() => setDeadlineToEdit({ ...deadlineToEdit, type })} className={`py-3 rounded-xl text-xs font-bold border transition-all ${deadlineToEdit.type === type ? 'bg-primary border-primary text-white' : (darkMode ? 'bg-slate-800 border-slate-700 text-slate-300' : 'bg-slate-50 border-slate-100 text-slate-600')}`}>
                          {type === 'tax' ? t('calendar.type_fiscal') : type === 'payment' ? t('calendar.type_payment_btn') : t('calendar.type_other_btn')}
                        </button>
                      ))}
                    </div>
